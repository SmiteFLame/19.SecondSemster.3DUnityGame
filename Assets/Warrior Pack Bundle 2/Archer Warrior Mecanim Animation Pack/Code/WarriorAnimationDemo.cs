...
public class WarriorAnimationDemo : MonoBehaviour
{
    ...
    public bool doubleclick_doing = false;
    public bool doubleclick_front = false;
    public bool doubleclick_back = false;
    public bool doubleclick_left = false;
    public bool doubleclick_right = false;
    float doubleclick_delaytime = 0.15f;
    float doubleclick_time = 0.0f;
    ...
    void Update()
    {
        ...
        if (!dead || !blockGui || !isBlocking)
        {
            if (!weaponSheathed)
            {
                ...
                if (!isBlocking)
                {
                    if (Input.GetKeyDown(KeyCode.V) && attack <= 3)
                    {
                        AttackChain();
                    }
                    if (!isInAir)
    ...
    void CameraRelativeInput()
    {
        if (!isStunned)
        {
            float inputHorizontal = 0; Input.GetAxisRaw("Horizontal");
            float inputVertical = 0; Input.GetAxisRaw("Vertical");
            ...
            if (Input.GetKeyDown(KeyCode.W))
            {
                if (doubleclick)
                {
                    if (doubleclick_front)
                    {
                        if (doubleclick_time < doubleclick_delaytime)
                        {
                            inputDashHorizontal = -1f;
                            doubleclick_doing = true;
                        }
                        else
                        {
                            SetDoubleClickFalse();
            ...
            if (!isBlocking)
            {
                ...
                if (inputVertical > 0.1 || inputVertical < -0.1 || inputHorizontal > 0.1 || inputHorizontal < -0.1)
                {
                    ...
                    if (Input.GetKeyDown(KeyCode.T))//Input.GetKeyDown(KeyCode.Q))// ||Input.GetKey(KeyCode.LeftShift) || )
                    {
    ...
    void SetDoubleClickFalse()
    {
        doubleclick = false;
        doubleclick_back = false;
        doubleclick_front = false;
        doubleclick_left = false;
        doubleclick_right = false;
    }
